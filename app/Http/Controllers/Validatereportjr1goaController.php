...
//die();
					...
					if($min_error=='')
					{
						foreach($rowData as $detail)
						{
							...
							$format_field='Number of Successful Gold Open Access Full-Text Article Requests by Month and Journal';
							...
							}else if($string_error==2)
							{
								...
								$error[$a]["error"]="Cell B1 should be Number of Successful Gold Open Access Full-Text Article Requests by Month and Journal";
								$a++;
								$data_error++;
							}
					...
					if($min_error=='')
					{
						foreach($rowData8 as $detail8){
							...
							}else if($string_error==2)
							{
								...
								$error[$a]["error"]="Cell H8 should be Reporting Period Total";
								$a++;
								$data_error++;
							}
							
							$input_field=$detail8['8'];
							$format_field='Reporting Period HTML';
							$string_error=$this->checkString($input_field,$format_field);
							if($string_error==1 )
							{
							...
							}else if($string_error==2)
							{
								...
								$error[$a]["error"]="Cell I8 should be Reporting Period HTML";
								$a++;
								$data_error++;
							}
							
							$input_field=$detail8['9'];
							$format_field='Reporting Period PDF';
							$string_error=$this->checkString($input_field,$format_field);
							if($string_error==1 )
							{
								$warning[$b]["data"]=$detail8['9'];
								$warning[$b]["error"]="Cell J8 contains no proper space";
								$data_warning++;
								$b++;
							}else if($string_error==2)
							{
								$error[$a]["data"]=$detail8['9'];
								$error[$a]["error"]="Cell J8 should be Reporting Period PDF";
								$a++;
								$data_error++;
							}
					...
					if($min_error=='')
					{
						foreach($rowData9 as $detail9){
							...
							/* if($detail9['1']==''){
								...
								$error[$a]["error"]="Cell B9 should be Blank";
								$a++;
								$data_error++;
							} */
							...
							if($detail9['2']==''){
								...
								$error[$a]["error"]="Cell C9 should be platform name";
								$a++;
								$data_error++;
							}
							...
							for($i=10;$i<=$highestRow;$i++)
							{
							$sheet->setCellValue($column.'7', "=sum(".$column."$i:".$column."$highestRow)");
						...
						for($i=10;$i<=$highestRow;$i++)
						{
							...
							while($column<=$highestColumn)
							{
								$cell = $sheet->getCell($column.$i)->getCalculatedValue();
								//echo $cell."</br>";
								//$sheet->setCellValue('B7', '=ISBLANK('.$column.$i.')');
								//$cell = $sheet->getCell('B7')->getCalculatedValue();
								//echo $cell."<br>";
								if($column=='D' || $column=='E' || $column=='B'){
